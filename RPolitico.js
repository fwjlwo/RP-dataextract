...
var pres = 0.13; // média de faltas do sistema
var priv = 1235235.03 // R$ 1.235.235,03 // média de uso da cota parlamentar no sistema


RankingsPoliticos = function(presenca, priv) {
	this.presenca = presenca || 0;
	this.priv = priv || 0;

	this.leis = [
		{
			id_lei: 'identificação da lei',
			tipo: 'tipo da lei',
			descr: 'descrição da lei'
		}
	]
}
RankingsPoliticos.constructor = RankingsPoliticos;
RankingsPoliticos.prototype = Object.create(Object.prototype);

rp = new RankingsPoliticos(pres, priv)

RPolitico = function(input_obj, rp) {

	obj = input_obj || {};

	this.rp = rp || new RankingsPoliticos();

	// dados do parlamentar
	this.nome = obj.nome || '';
	this.CPF = obj.CPF || '';
	this.infosite = obj.infosite || '';
	this.estado = obj.estado || '';
	this.cargo = obj.cargo || '';
	this.apelido = obj.apelido || '';
	this.nascimento = obj.nascimento || '';
	this.formacao = obj.formacao || '';
	this.filpardidaria = obj.filpardidaria || '';
	this.email = obj.email || '';
	this.partido = obj.partido || '';
	this.profissao = obj.profissao || '';
	this.orgao = obj.orgao || '';
	// sessão presença
	this.presenca = (obj.presenca) ? _.clone(obj.presenca) : {
		tipo: '',
		ano: '',
		sessoes: 0,
		presenca: 0,
		just: 0,
		njust: 0,
		pontosRP: [] // {data: data, p: num}
	};
	// sessão privilégios
	this.priv = (obj.priv) ? _.clone(obj.priv) : {
		cota_parlamentar: [
			{
				ano: '',
				valores: [] // {tipo: descrição, val: valor em reais}
			}
		],
		pontosRP: [] // {data: data, p: num}
	};
	// sessão processos judiciais
	this.proc = (obj.proc) ? _.clone(obj.proc) : {
		lista: {},
		pontosRP: [] // {data: data, p:num}
	};

	// outros criterios
	this.outros = (obj.outros) ? _.clone(obj.outros) : {};

	// Qualidade parlamentar
	this.qual = (obj.qual) ? _.clone(obj.qual) : [
		/* { // formato das proposicoes
			name_id: 'nome de identificação da lei'
			voto: string (s = sim, n = nao, a = abstenção)
		} */
	]
};
RPolitico.constructor = RPolitico;
RPolitico.prototype = Object.create(Object.prototype);

RPolitico.prototype.calcPresenca = function() {
	var media = this.rp.presenca;
	var p = this.presenca;
	var coef = (p.just + 2 * p.njust) / p.sessoes;
	var pts = Math.round( ( (( coef / media ) - 1) / 0.5 ) * -10 * 2);
	//
	return pts;
}

RPolitico.prototype.calcPriv = function() {
	var media = this.rp.priv;

	// soma de todas as cotas parlamentares
	var total = _.map(this.priv.cota_parlamentar, function(ano) {
					return _.reduce(ano.valores, function(memo, num){ return memo + num.val; }, 0)
				})
				.reduce( function(memo, num){ return memo + num; }, 0);

	var pts = ( ( (total / media) - 1) / 0.5 ) * -10 * 3;
}
